...
const {bundle, run, html, assertBundleTree} = require('./utils');
...
function test (file, assert) {
  it(file, done => {
    ...
    fs.writeFileSync(mockEntry+'.html', `
<html>
<head>
</head>
...
`);
    ...
    .then(b => {
      ...
      let dom = new jsdom.JSDOM(src, {pretendToBeVisual: true, resources: "usable", runScripts: "dangerously", virtualConsole});
      dom = dom;
...
function assertRenderFn (options, template) {
  ...
  str2 = str2.replace(/function [a-z_]+/g, 'function ').replace(/[']/g, '"').replace(/[ ]+/g, '');
...
describe('vue', function() {
  test('basic', window => {
    ...
    const style = fs.readFileSync(window.document.querySelector('link').href).toString();
  ...
  test('scoped-css', window => {
    var module = window.vueModule
    var id = 'data-v-' + genId(require.resolve('./integration/vue/scoped-css.vue'))
  ...
  test('media-query', window => {
    const style = fs.readFileSync(window.document.querySelector('link').href).toString();
    var id = 'data-v-' + genId(require.resolve('./integration/vue/media-query.vue'))
